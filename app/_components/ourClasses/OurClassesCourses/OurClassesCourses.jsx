...
const OurClassesCourses = () => {
  ...
  const handleSecondaryButtonChange = (index, value) => {
    const updated = [...secondaryButtons];
    updated[index] = value;
    setSecondaryButtons(updated);
  };

  const addSecondaryButtonField = () => {
    setSecondaryButtons([...secondaryButtons, ""]);
  };

  const removeSecondaryButtonField = (index) => {
    setSecondaryButtons(secondaryButtons.filter((_, i) => i !== index));
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const allEmpty =
      formValues.courses.every((course) =>
      ...
      primaryButtonText === "" &&
      primaryButtonUrl === "" &&
      secondaryButtons.every((val) => val === "") &&
      secondaryButtonUrl === "";
    ...
    <div className="md:p-8 md:ml-64 pt-[4.5rem] md:pt-26 2xl:pt-30 bg-zinc-950">
      ...
      >
        ...
        <div className="border border-gray-600 rounded-lg p-4 space-y-4 bg-zinc-800">
          <h1 className="text-white mb-3">Add Filter Category Buttons</h1>
          ...
          {secondaryButtons.map((text, index) => (
            <div key={index} className="flex  gap-2 items-center">
              <input
                ...
                value={text}
                onChange={(e) =>
                  handleSecondaryButtonChange(index, e.target.value)
                }
                placeholder={`Category Button ${index + 1}`}
                className="border border-gray-300 px-3 py-2 rounded w-70"
              />
              {secondaryButtons.length > 1 && (
                <button
                  type="button"
                  onClick={() => removeSecondaryButtonField(index)}
          ...
          <button
            type="button"
            onClick={addSecondaryButtonField}
            className="mt-3 text-yellow-400 hover:text-yellow-300 flex items-center gap-1"
          >
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    {formValues.courses.length > 1 && (
                      <button
                        ...
                        className="text-red-400 hover:text-red-300 global-btn"
                      >
        ...
        {/* Buttons Section */}
        <div className="pt-8">
          <h2 className="feature-title">Customize Card Buttons</h2>
